...
def generate_smiles_conditional(
    ...
    with torch.no_grad():
        for _ in range(max_len):
            ...
            # mask invalid tokens
            logits[0, tokenizer.start_token] = -float('inf')
            logits[0, tokenizer.pad_token] = -float('inf')
...
def check_sequences_conditional(
    SA_model, TPSA_model, QED_model, LogP_model, CSP3_model,
    gen_model, tokenizer,
    ...
    for m in [SA_model, TPSA_model, QED_model, LogP_model, CSP3_model, gen_model]:
        m.to(device)
        m.eval()
    ...
    print("-" * 30)
    print(f"Valid smiles = {valid_smiles}/{num_samples}")
    print(f"Avg SA = {np.mean(SA_scores):.4f}")
    print(f"Avg TPSA = {np.mean(TPSA_scores):.4f}")
    print(f"Avg QED = {np.mean(QED_scores):.4f}")
    print(f"Avg LogP = {np.mean(LogP_scores):.4f}")
    print(f"Avg CSP3 = {np.mean(CSP3_scores):.4f}")
    print("-" * 30)
