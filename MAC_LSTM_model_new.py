...
# Other modules
import datetime # for timestamped model saving
...
class SmilesGeneratorEnv(EnvBase):
    ...
    def __init__(
        ...
        #ADDED: Pick device in a safe way: CUDA if available, else CPU
        if device is None:
            device = torch.device(
    ...
    def _step(self, tensordict):
        ...
        if done:
            ...
            for tag, smiles in self.tag_to_smiles.items(): # self.tags_to_smiles
...
if __name__ == "__main__":
    ...
    SA_model.load_state_dict(
        torch.load("models/SA_scores_weights.pth", map_location=device)
    )
    ...
    TPSA_model.load_state_dict(
        torch.load("models/TPSA_scores_weights.pth", map_location=device)
    )
    ...
    QED_model.load_state_dict(
        torch.load("models/QED_scores_weights.pth", map_location=device)
    )
    ...
    LogP_model.load_state_dict(
        torch.load("models/LogP_scores_weights.pth", map_location=device)
    )
    ...
    CSP3_model.load_state_dict(
        torch.load("models/CSP3_scores_weights.pth", map_location=device)
    )

    # Generator
    gen_model.load_state_dict(
        torch.load("models/model_gen_weights_perfect.pth", map_location=device)
    )
    ...
    gen_model.load_state_dict(
        torch.load("models/model_RL_gen_weights.pth", map_location=device)
    )
