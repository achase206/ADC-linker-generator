...
#BROKEN?
#Testing new tokenizer & dataset parameters-- 
def kfolds_LSTM_scores(df, tag_map, score_type="SA"):
    """
    df = dataframe
    tag_map = output of tag_dataset_with_motifs
    score_type = "SA", "TPSA", "QED", "LogP", or "CSP3"
    """

    print("Initializing tokenizer...")
    tokenizer = Tokenizer(df, tag_map)

    print("Building condition dictionaries...")
    stoi_dicts, itos_dicts, dict_length = conditions_tokens(df)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        train_loader = DataLoader(
            train_subset, batch_size=64, shuffle=True, collate_fn=tokenizer.collate_smiles
        )
        val_loader = DataLoader(
            val_subset, batch_size=64, shuffle=False, collate_fn=tokenizer.collate_smiles
        )
        ...
        for epoch in range(num_epochs):
            ...
            for i, (smile, ab, pay, target, score) in enumerate(train_loader):
        ...
        with torch.no_grad():
            for smile, ab, pay, target, score in val_loader:
                smile = smile.to(device)
                score = score.to(device)
